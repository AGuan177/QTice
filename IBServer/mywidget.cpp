...
{
    this->addMessageToTextBrowser(QString::number(handle) + " connect");
    //将信息插入m_socketMap
...
{
    addMessageToTextBrowser(QString::number(handle) + " disconnect");
...
void MyWidget::slot_readData(int handle, const QByteArray &data)
{
    ...
    addMessageToTextBrowser("接受:" + str);
    ...
    if(m_filemap.value(handle)->istransporting==true)
    {
        ...
        addMessageToTextBrowser("写入服务器的消息" + QString(data));

        //如果文件的数据收齐
        if( m_filemap.value(handle)->receivefilesize==m_filemap.value(handle)->fileszie)
        {
          addMessageToTextBrowser("文件传输完毕");
          //关闭文件传输
    ...
    else if(str.startsWith("#21")==true)
    {
        ...
        addMessageToTextBrowser("收到"+str_1+"建立"+str_2+"的申请");
        //要创建的群名不存在
    ...
    else if(str.startsWith("#22#")==true)
    {
        QString str_1=str.section('/',1,1);//申请者id
        QString str_2=str.section('/',2,2);//群名
        addMessageToTextBrowser("收到"+str_1+"加入"+str_2+"的申请");
    ...
    else if(str.startsWith("#31#"))
    {
        ...
        addMessageToTextBrowser("我收到了你发来的文件信息：" + m_filemap.value(handle)->filename + " 大小为：" + m_filemap.value(handle)->fileszie);
    }
    //更换头像
    else if(str.startsWith("#41#"))
    {
        ...
        if(query.exec(cmd_1)) addMessageToTextBrowser("头像更换成功");
    }
}
...
void MyWidget::addMessageToTextBrowser(QString message)
{
...
void MyWidget::sendfiletoId()
{
    ...
    addMessageToTextBrowser("总共发出了" + QString::number( allou) + "的数据");
    file.close();//关闭文件
}
